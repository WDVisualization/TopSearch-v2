...
def analysisview(request):
    return render(request, 'toptwenty_analysis.html')


def toptwenty(request):
    try:
        sql = "SELECT * FROM `weibo` where rank=1 group by keywords ORDER BY flow DESC limit 0,20"
        cursor.execute(sql)
        datas = cursor.fetchall()
        db.commit()
        objects_list = []
...
def changelists(request):
    try:
        kwd = request.GET['keywords']
        if len(kwd) > 0:
            sql = "SELECT * FROM `weibo` where keywords = "+kwd+" ORDER BY created_at"
            cursor.execute(sql)
            datas = cursor.fetchall()
            db.commit()
        objects_list = []
